...
for service, steps in services.items():
    total_cost = sum([procedures[step][0] for step in steps])
    total_commission = sum([procedures[step][1] for step in steps])
    results[service] = {"Valor Total": total_cost, "Comissão": total_commission, "Porcentagem de Comissão": total_commission/total_cost}
